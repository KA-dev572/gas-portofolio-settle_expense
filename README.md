...
## 制作の背景
- 経費精算業務では、申請内容をもとに仕訳データを作成する工程において、手作業による転記や確認が発生しやすく、ミスや属人化の原因になりがちです。
- 本制作物は、そうした業務フローを想定し、定型的な処理を自動化しつつも、人が内容を確認・修正できる余地を残すことを目的として作成しました。
...
## 課題・改善の余地
- 勘定科目の入力を申請者側にさせているが、実務的に不適切なら修正が必要。
- setValuesなどAPI呼び出し回数は減らすことができる。
- エラー対策の呼び出しを正確な形に。（現状だとGoogle フォーム側でエラーが起きない形にしているので動作は問題ない。）
